    ...
    def _assign_to_new_stickers(self, moas_to_be_on_new_sticker, proposal, stickers_to_be_returned, stickers_to_be_replaced_for_renewal=[]):
        ...
        for moa_to_be_on_new_sticker in moas_to_be_on_new_sticker:
            ...
            if not new_sticker or new_sticker.mooringonapproval_set.count() % 4 == 0:
                ...
                #if no proposal has been provided it is either a swap or something has gone wrong
                #substitute the vessel ownership with that on the latest proposal for this approval
                sub_vessel_ownership = None
                if not proposal:
...
class Sticker(RevisionedMixin):
    ...
    STATUSES_FOR_NON_EXPORTED_FILTER = (
...
reversion.register(AnnualAdmissionPermit, follow=[
    'approval'
])
reversion.register(AuthorisedUserPermit, follow=[
    'approval'
])
reversion.register(MooringLicence, follow=[
    'approval'
])

reversion.register(Sticker, follow=['mooringonapproval_set'])
